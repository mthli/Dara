...
public class MenuSheetView extends FrameLayout {
    ...
    public MenuSheetView(final Context context, final MenuType menuType, @Nullable final CharSequence title, final OnMenuItemClickListener listener) {
        ...
        inflate(context, menuType == MenuType.GRID ? flipboard.bottomsheet.commons.R.layout.grid_sheet_view : flipboard.bottomsheet.commons.R.layout.list_sheet_view, this);
        absListView = (AbsListView) findViewById(menuType == MenuType.GRID ? flipboard.bottomsheet.commons.R.id.grid : flipboard.bottomsheet.commons.R.id.list);
        if (listener != null) {
        ...
        // Set up the title
        titleView = (TextView) findViewById(flipboard.bottomsheet.commons.R.id.title);
        originalListPaddingTop = absListView.getPaddingTop();
        setTitle(title);

        ViewCompat.setElevation(this, DisplayUtils.dp2px(getContext(), 16f));
    }
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            setOutlineProvider(new DisplayUtils.ShadowOutline(w, h));
        }
    }
            ...
            if (item.isVisible()) {
                if (item.hasSubMenu()) {
                    ...
                    if (subMenu.hasVisibleItems()) {
                        ...
                        if (menuType == MenuType.LIST && i != menu.size() - 1) {
                            items.add(SheetMenuItem.SEPARATOR);
                        }
                    }
                } else {
                    int groupId = item.getGroupId();
                    if (groupId != currentGroupId && menuType == MenuType.LIST) {
                        items.add(SheetMenuItem.SEPARATOR);
                    }
                    items.add(SheetMenuItem.of(item));
    ...
    public void setTitle(CharSequence title) {
        if (!TextUtils.isEmpty(title)) {
            ...
            absListView.setPadding(absListView.getPaddingLeft(), originalListPaddingTop + (int) DisplayUtils.dp2px(getContext(), 8f), absListView.getPaddingRight(), absListView.getPaddingBottom());
        }
    }
    ...
    private class Adapter extends BaseAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            switch (viewType) {
                case VIEW_TYPE_NORMAL:
                    ...
                    if (convertView == null) {
                        convertView = inflater.inflate(menuType == MenuType.GRID ? flipboard.bottomsheet.commons.R.layout.sheet_grid_item : R.layout.layout_sheet_item, parent, false);
                ...
                case VIEW_TYPE_SUBHEADER:
                    if (convertView == null) {
                        convertView = inflater.inflate(flipboard.bottomsheet.commons.R.layout.sheet_list_item_subheader, parent, false);
                    }
                ...
                case VIEW_TYPE_SEPARATOR:
                    if (convertView == null) {
                        convertView = inflater.inflate(flipboard.bottomsheet.commons.R.layout.sheet_list_item_separator, parent, false);
                    }
                    break;
            }
        ...
        class NormalViewHolder {
            ...
            NormalViewHolder(View root) {
                icon = (ImageView) root.findViewById(flipboard.bottomsheet.commons.R.id.icon);
                label = (TextView) root.findViewById(flipboard.bottomsheet.commons.R.id.label);
            }
